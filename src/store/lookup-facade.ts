...
export async function lookup(store: Store): Promise<boolean> {
  ...
  logger.debug(`[${store.name}] starting product availability check for ${store.links.length} links`);
 ...
 */
async function checkLink(browserSession: BrowserSession, store: Store, link: Link): Promise<boolean> {
  return await browserSession.tryWithPage(async (page) => {
...
function shouldCheckLink(link: Link): boolean {
  ...
  // Check price filter
  if (config.store.maxPrice.series[link.series] && link.price &&
