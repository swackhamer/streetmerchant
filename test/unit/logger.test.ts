...
describe('Logger Module', () => {
  ...
  describe('Print formatters', () => {
    it('should format backoff message correctly', () => {
      ...
      // Assert
      expect(result).toBe(
        '✖ [test-store] [nvidia (3080)] test-model :: BACKOFF DELAY status=429 delay=5000'
      );
    });
    ...
    it('should format maxPrice message correctly', () => {
      ...
      // Assert
      expect(result).toBe(
        '✖ [test-store] [nvidia (3080)] test-model :: PRICE 1500 EXCEEDS LIMIT 1000'
      );
    });
    ...
    it('should format recursionLimit message correctly', () => {
      ...
      // Assert
      expect(result).toBe(
        '✖ [test-store] [nvidia (3080)] test-model :: CLOUDFLARE RETRY LIMIT REACHED, ABORT'
      );
    });
  });
